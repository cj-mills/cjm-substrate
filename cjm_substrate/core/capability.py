...
class ToolCapability(ABC):
    ...
    class-identical `ToolCapability` alias until the Option C migration
    cascade split them (the alias was REMOVED at SG-48).
    ...
    `get_current_config`. The rest are overridable concrete defaults, grouped:
    lifecycle (`prefetch` / `cleanup` + the `on_disable` / `on_enable` signal
    hooks), configuration (`get_config_options` / `reconfigure` +
    `RELOAD_TRIGGER` declarative releases via `reconfigure_with_triggers` /
    `fields_that_changed`), cancellation (`cancel` / `check_cancel` /
    `register_cancel_callback` / `cancel_signal_to`), observability
    (`report_progress` / `report_usage` / `heartbeat`), and multi-op dispatch
    (`dispatch_to_action`); `execute_stream`'s transitional default remains
    for fused-era capabilities only.
    """
