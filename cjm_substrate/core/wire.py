...
protocol, the typed result envelope that carries task results across the
worker HTTP/JSON boundary (retiring the per-consumer field_of dict-or-object
tolerance helpers; pass-2 Thread 3, evidence E5/D10/C7), the per-call
CallEnvelope identity/control block (CR-14/CR-15), and the in-worker account
capture that rides back as the X-CJM-Accounts response header. Design
rationale: on-graph note wire-design-provenance."""
...
class FileBackedDTO(Protocol):
    ...
    When the Proxy detects an argument implementing this protocol, it calls
...
def flat_from_dict(
    ...
    on-graph note wire-design-provenance) and lets the constructor enforce required fields
