"""Resource-aware multi-lane job queue for capability execution (stage 3 / CR-16 rework).

Priority dispatch (higher first, FIFO within a priority) across independent lanes: jobs co-run when the admission ladder allows — derived from empirical resource records + live sysmon telemetry, never from declared metadata; no evidence means the job runs exclusive (its first run is the measurement run). Compositions — DAGs of capability invocations with execution-time-bound OutputRef inputs — replaced submit_sequence outright; member Jobs are created lazily as dependencies complete. Cancellation is cooperative with graceful fallback to force termination; progress/status ride a poll loop during execution. Observability is journal-primary (CR-14): the push-based event bus is a live tail of the journal, with typed accessors + exact per-job diagnostics for UI integration."""
...
from typing import (Any, AsyncIterator, Dict, List, Optional, Protocol, runtime_checkable, Set,
                    Tuple, TYPE_CHECKING)
...
from cjm_substrate.core.ports import (Composition, CompositionBindingError, CompositionRun,
                                      new_composition_run, NodeState, resolve_node_kwargs)
...
# JobQueue stays inline in cjm-substrate for v1; the Protocol enables clean
# future extraction.
if TYPE_CHECKING:
...
class ResourceSnapshot:
    ...
    Field shape mirrors `cjm-fasthtml-job-monitor`'s pre-substrate
    ResourceSnapshot so the cascade migration to the substrate-owned type is
...
class JobQueue:
    ...
    async def submit(
        ...
        **kwargs
        ...
        # chain minimal).
        from cjm_substrate.core.errors import CapabilityDisabledError, CapabilityInputError
    ...
    async def submit_composition(
        ...
        """
        from cjm_substrate.core.errors import CapabilityDisabledError
