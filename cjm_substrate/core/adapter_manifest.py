...
def match_protocol_against_surface(
    ...
    Annotations are recorded in both manifests for DIAGNOSTICS but never
    compared: cross-env rendering of type annotations differs
    (`List[GraphNode]` vs fully-qualified forms), so signature-string equality
    would false-negative on compatible pairs. Known accepted edge: a surface
    method absorbing protocol params via `**kwargs` registers a param
    mismatch — no current protocol needs it.
    """
