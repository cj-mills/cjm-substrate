...
class ResourceSample:
    ...
    V1 limitation: the cpu/memory/gpu fields come from `proxy.get_stats()` at the
    moment we measure — a point-in-time proxy for peak ("what was the worker using
    when execute returned"), not the true peak during execute. Worker-side peak
    tracking is a future enhancement when needed.
    """
...
class EmpiricalResourceStore(Protocol):
    ...
    if time-series queries become necessary. `@runtime_checkable` so
    `isinstance(x, EmpiricalResourceStore)` asserts the seam without importing
    the concrete class.
    """
...
class LocalEmpiricalResourceStore:
    ...
    Welford v1 deliberately omits the variance accumulator M2 — no consumer
    exposes variance; adding it later is a schema-additive change.
    """
    ...
    def record_sample(
        ...
        Welford update for each mean. Max-of-peaks for memory metrics — the
        worst observation drives eviction-candidate selection, while the running
        mean smooths over noise for UI hints / capacity-planning.
