...
The worker subprocess may spawn further subprocesses (e.g. Voxtral-vLLM launches an OpenAI-compatible vLLM server as a grandchild of the substrate), and per-PID GPU attribution that matches only the worker's own PID misses those grandchildren — underreporting GPU usage for any subprocess-spawning capability. The fix: attribute GPU memory across the worker's process SUBTREE — the worker's /stats endpoint reports its subtree PIDs (essentially free: it already walks proc.children(recursive=True) for the RAM sum), and the substrate intersects that set with the system-monitor capability's list_processes() enumeration of GPU-holding PIDs."""
