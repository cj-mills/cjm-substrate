...
from cjm_substrate.core.diagnostics_store import (DiagnosticsStore, LocalDiagnosticsStore,
                                                  normalize_stream_line, StreamChunk)
from cjm_substrate.core.errors import (CapabilityCancelledError, CapabilityFatalError,
                                       CapabilityInputError, CapabilityResourceError,
                                       CapabilityTransientError, ResourceShortfall, WorkerOOMError)
...
class RemoteCapabilityProxy(ToolCapability):
    ...
    The manifest dict must carry at minimum the keys the proxy reads:
    `python_path` (the isolated env's interpreter), `module` + `class` (the
    capability to load in-worker), and optionally `name`, `version`, and
    `env_vars` (static worker-env defaults; CR-12's `extra_env` overlay wins
    over them at spawn).

    Constructing the proxy STARTS the worker; use it as a (sync or async)
    ...
    # Execute methods — Track A classification on httpx faults. Httpx
    # exceptions caught: ConnectError (worker stopped accepting connections),
    # RemoteProtocolError (worker died mid-response), ReadError / WriteError
    # (socket reset mid-payload). Other httpx exceptions (timeout etc.) pass
    # through unchanged.
    def execute(self, *args, **kwargs) -> Any:
        """Execute the capability synchronously.

        CR-4: HTTP 409 from the worker maps to a typed
        `CapabilityCancelledError` so callers can distinguish cooperative
        cancellation from a real failure; other non-200s go through the G7
        typed unary error channel (`_raise_typed_execute_error`).

        CR-7 Track A: catches httpx connection / protocol faults, calls
        `_check_worker_death`, and either raises a typed `WorkerOOMError` /
        `CapabilityTransientError` or re-raises the original httpx error
        (worker still alive — caller treats as a generic transient network
        issue).
        """
        payload = self._prepare_payload(args, kwargs)
        try:
    ...
    async def execute_async(self, *args, **kwargs) -> Any:
        """Execute the capability asynchronously. Same CR-4 409 mapping and
        CR-7 Track A fault-classification semantics as `execute`; rides the
        persistent per-loop keep-alive client (`_ensure_async_client`)."""
        payload = self._prepare_payload(args, kwargs)
        try:
