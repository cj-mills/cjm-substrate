...
from cjm_substrate.core.adapter_manifest import is_adapter_manifest
from cjm_substrate.core.config import CondaType, get_config, load_config, set_config
from cjm_substrate.core.manifest_format import (CodeSection, compute_config_schema_hash,
                                                compute_structural_surface_hash, DriftTracking,
                                                InstallSection, load_manifest, ManifestV2,
                                                write_manifest)
from cjm_substrate.core.metadata import ResourceRequirements
from cjm_substrate.core.platform import (build_conda_command, conda_env_exists, download_micromamba,
...
def setup_runtime(
    ...
    Required before `install-all` when the configuration sets
    `conda_type: micromamba`; the other conda types use runtimes already
    on PATH.
    """
    cfg = get_config()
...
def regenerate_manifest(
    ...
    Package-source recovery order: `--package <spec>` operator override always
    wins; then the manifest's own `package_source` field; then a
    `--capabilities capabilities.yaml` lookup by capability name (legacy
    manifests without `package_source`); else error with an actionable message.
    The ecosystem-wide cascade (this command looped over every capability)
    lives in `scripts/cascade_manifests.py`.
    """
    cfg = get_config()
